...
interest_rates = np.vstack(interest_rates[:,2])
inflation = np.genfromtxt("inflation.csv", delimiter = ",", skip_header=1)
inflation = np.vstack(inflation[:,2])
#-------------------------------------------------------------------------------
### Ridge Regression based solely off precipitation
#-------------------------------------------------------------------------------
# Add a column of ones to account for the bias in the data
trainPercent = .8
numberForTrain = int(trainPercent * len(interest_rates))
numberForTest = len(interest_rates) - numberForTrain

trainData = interest_rates[:numberForTrain]
testData = interest_rates[numberForTrain:]
